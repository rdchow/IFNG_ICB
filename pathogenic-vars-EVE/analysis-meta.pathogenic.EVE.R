...
data=read.table("FINAL-DATASET.pretreat.pathogenic.EVE.v2.txt",sep="\t",header=TRUE)

# remove patients with non-pathogenic other alterations
data = data[!(data$PredPathogenicAlt_EVE == 0 & data$AnyAlt == 1),] #removes 16 patients
...
data$PredPathogenicAlt_EVE = factor(data$PredPathogenicAlt_EVE,levels=c("0","1"))
...
for (i in 1:length(cohorts)){
    ...
    if (all(mydat$Type == "solo")){ 
        if (sum(is.na(mydat$TMB)) == 0){ # if TMB available for all tumors 
            ...
            mylm = logistf(BooleanResponse ~ PredPathogenicAlt_EVE + TMBbin,data=mydat)
            mycoefs = as.data.frame(t(coef(mylm)))
            mydf[i,3] = mycoefs$PredPathogenicAlt_EVE1
            mydf[i,8] = mycoefs$TMBbin1

            myerrors= as.data.frame(t(sqrt(diag(vcov(mylm)))))
            mydf[i,4] = myerrors$PredPathogenicAlt_EVE1
            mydf[i,9] = myerrors$TMBbin1

            myconfs = as.data.frame(t(confint(mylm)))
            mydf[i,5] = round(myconfs$PredPathogenicAlt_EVE1[1],digits=3)
            mydf[i,6] = round(myconfs$PredPathogenicAlt_EVE1[2],digits=3)
            mydf[i,10] = round(myconfs$TMBbin1[1],digits=3)
            mydf[i,11] = round(myconfs$TMBbin1[2],digits=3)

            mypvals = as.data.frame(t(mylm$prob))
            mydf[i,7] = mypvals$PredPathogenicAlt_EVE1
            mydf[i,12] = mypvals$TMBbin1

        } else { # if missing TMB data (only Gao cohort)
            mylm = logistf(BooleanResponse ~ PredPathogenicAlt_EVE,data=mydat)
            mycoefs = as.data.frame(t(coef(mylm)))
            mydf[i,3] = mycoefs$PredPathogenicAlt_EVE1
            mydf[i,7] = NA
...
write.table(mydf,"regression-results.pretreat.pathogenic.removeNonpathMutated.EVE.v2.txt",sep="\t",row.names=FALSE)
...
mydf = read.table("regression-results.pretreat.pathogenic.removeNonpathMutated.EVE.v2.txt",sep="\t",header=TRUE)
so = read.table("sample-order.EVE.txt",sep="\t",header=TRUE)
...
pdf("forest-plot-multilevelReg.pretreat.pathogenic.removeNonpathMutated.EVE.v2.pdf",height=7.5,width=8,useDingbats=FALSE)
